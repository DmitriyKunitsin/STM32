...
//				uint8_t calc_crc_result = CalcCRC8SKLP_Load(rxBuffer->data, indexCRC, rxBuffer->data[indexCRC]);
				...
				/*
				--------------------------------------------------
				Считаем CRC8 полученного пакета по Manchester
				--------------------------------------------------
				*/
				uint8_t calc_crc_result = CalcCRC8SKLP(rxBuffer->data, sizeAllPacket);
				/*
				--------------------------------------------------
				Если CRC8 посчиталось верно и сошлось идём дальше
				--------------------------------------------------
				*/
...
//				bool ReceiverValid = SKLP_ReceivePacket(aTmpBuff, sizeoff(aTmpBuff), rxBuffer->data, rxBuffer->count);
				...
				*/
				...
				/*
				----------------------------------------------------
				Ждем пока пакет по UART не отправится полностью
				----------------------------------------------------
				*/
				...
				*/
				...
				/*
				-----------------------------------------------
				Получили ответ от МПИ
				-----------------------------------------------
				*/
				static bool ReceiverValid = false;
				switch ( Message.Event )
					{
						case EVENT_SKLP_ANSWER:// Ответ на запрос
								...
								/*
								-----------------------------------------------------
								Изменение конфигурации портов для записи
								-----------------------------------------------------
								*/
								...
								/*
								------------------------------------------------------------
								Имитация отправки на мачестер, но отправка на 3 UART, того
								ответа, что предоставила МПИ
								------------------------------------------------------------
								...
								*/
								...
								/*
								-------------------------------------------------------------
								Отрпавка по манчестеру ответа из сформированного пакета
								-------------------------------------------------------------
								*/
									uint16_t size_output_packet = (Message.Packet.Size % 2) == 0 ? Message.Packet.Size : Message.Packet.Size + 1;
									for(int i = 0; i < size_output_packet; i+=2) {
