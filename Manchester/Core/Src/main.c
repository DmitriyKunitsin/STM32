...
//	HAL_TIM_PWM_Start(&htim2, TIM_CHANNEL_4);
  ...
  /* USER CODE BEGIN WHILE */
	int cnt = 0;
	while (1) {
		//pushDatesToPort(0b10101010, 0b10101010); // отправка по кабелю
		cnt = (cnt + 1)%4;
		const uint16_t aBuff[4] = { 0xAAAA, 0x0000, 0x5555, 0xFFFF };
		pushDatesToPort(aBuff[cnt] & 0xFF, (aBuff[cnt]>>8) & 0xFF); // отправка по кабелю

//		pushDatesToPort(0xFF, 0x00);
    /* USER CODE END WHILE */
...
/* USER CODE END MX_GPIO_Init_1 */
  ...
  HAL_GPIO_WritePin(GPIOA, DA0_Pin|DA1_Pin|DA2_Pin|DA3_Pin
                          |DA4_Pin|DA5_Pin|DA6_Pin|DA7_Pin, GPIO_PIN_RESET);
  ...
  /*Configure GPIO pin : ACHD_Pin */
  GPIO_InitStruct.Pin = ACHD_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  HAL_GPIO_Init(ACHD_GPIO_Port, &GPIO_InitStruct);
