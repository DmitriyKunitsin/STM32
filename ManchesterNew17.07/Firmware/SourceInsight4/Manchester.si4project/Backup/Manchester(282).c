...
/*Функция Ждет 5 миллисекунд ответ по USART, если не дожидается, то возвращает 0
 * Если функция дожидается пакет информации с USART, то записывает его в массив и возвращает 1*/
...
//	uint32_t currentTime = startTimer;
//	uint8_t packetTrue = 0;
//	uint8_t flag = 0;
//	while ((currentTime - startTimer) < 5) {
//		if (checkDateForUSART == 1) {
//			for (int i = 0; i < 200; ++i) {
//				HAL_UART_Receive(&huart1, &pData[i], 1, HAL_MAX_DELAY);
//				__HAL_TIM_SET_COUNTER(&htim6, 0);
//				while (__HAL_TIM_GET_COUNTER(&htim6) < 6900) {
//					if (checkDateForUSART == 1) {
//						HAL_UART_Receive(&huart1, &pData[i], 1,
//						HAL_MAX_DELAY);
//						checkDateForUSART = 0;
//						flag = 1;
//					}
