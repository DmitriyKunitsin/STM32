...
void change_Configure_GPIO_Read(GPIO_InitTypeDef *GPIO) {
	...
	GPIO->Pin = DA0_Pin | DA1_Pin | DA2_Pin | DA3_Pin | DA4_Pin | DA5_Pin | DA6_Pin | DA7_Pin | AINTRX_Pin | AINTTX_Pin;
	...
	/* Пин для 588ВГ6, сообщает, что целостный пакет из 20 байт */
	GPIO->Pin = ACHD_Pin;
	GPIO->Mode = GPIO_MODE_INPUT;
	GPIO->Pull = GPIO_NOPULL;
	HAL_GPIO_Init(GPIOB, GPIO);
...
void change_Conffigure_GPIO_Write(GPIO_InitTypeDef *GPIO) {
	...
	GPIO->Pin = DA0_Pin | DA1_Pin | DA2_Pin | DA3_Pin | DA4_Pin | DA5_Pin | DA6_Pin | DA7_Pin;
	GPIO->Mode = GPIO_MODE_OUTPUT_PP;
	GPIO->Pull = GPIO_NOPULL;
	GPIO->Speed = GPIO_SPEED_FREQ_LOW;
	HAL_GPIO_Init(GPIOA, GPIO);
...
static void SKLP_SlaveTask( void *pParameters )
{
	...
	change_Configure_GPIO_Read(GPIO);
...
//				if(TX флаг с 588ВГ6 дернулся) {
//					качаем два байта в буфер
//					t = 0;
