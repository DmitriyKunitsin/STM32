...
//			rxBuffer->rxEventFull = 0;
			uint32_t sizePacket = rxBuffer->count;
			if (sizePacket > 0) {
					if (HAL_UART_Ext_Transmit(aSKLP_Interfaces[iSKLP_Machester].pUART_Hdl, rxBuffer->data, sizePacket, EVENT_UART_EXT_TX_COMPLETE) != HAL_OK) {
						// Обработка ошибки передачи
					}

//			HAL_UART_Ext_Transmit(aSKLP_Interfaces[iSKLP_Machester].pUART_Hdl, rxBuffer->data, sizePacket, EVENT_UART_EXT_TX_COMPLETE );
			rxBuffer->count = 0;
