...
//			uint16_t sizePacket = rxBuffer->count;
			if(rxBuffer->count > 0) {
				...
				uint8_t calc_crc_result = CalcCRC8SKLP_Load(rxBuffer->data, sizeAllPacket, rxBuffer->data[indexCRC]);
				for(int i = 0; i < 1024; ++i) {
						rxBuffer->data[i] = 0;
					}
				if(0xB4 == calc_crc_result) {
...
//				bool ReceiverValid = SKLP_ReceivePacket(aTmpBuff, sizeoff(aTmpBuff), rxBuffer->data, rxBuffer->count);
			...
			HAL_UART_Ext_Transmit(aSKLP_Interfaces[iSKLP_USART].pUART_Hdl, rxBuffer->data, rxBuffer->count, EVENT_UART_EXT_TX_COMPLETE );
			GPIO_Common_Toggle(iGPIO_KT2);
			GPIO_Common_Toggle(iGPIO_KT2);
