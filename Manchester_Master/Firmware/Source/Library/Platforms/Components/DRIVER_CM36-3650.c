// Драйвер мотора CM36-3650
// Черный - "-" питание
// Красный - "+" питание
// Синий - управление скорость (ШИМ 0- 5В, с частотой 20 кГц). Чем больше, коэффициент заполнения, тем быстрее крутится мотор
// Белый - управление направлением вращения
// Желтый - контроль скорости вращения (6 импульсов на оборот вала мотора)
...
#include "DRIVER_CM36-3650.h"		// родной
...
typedef struct CM36_3650_Motor_struct{
	float CurrentSpeed;
	float CurrentAngle;
...
void Motor_SpeedCounter_Init(void)
{
	...
	*pTim_Init = (TIM_Base_InitTypeDef){0};
	...
	pTim_Init->Period = 			ARR_VALUE_CM36_3650;
...
void CM36_3650_Motor_Stop(void){
	LL_TIM_OC_SetCompareCH1(MOTOR_SPEED_CONTROL_TIM, 0.0f);
...
bool CM36_3650_Motor_Angle_Set(int16_t angle, float speed_percent)
{
	...
	uint32_t PulsesPerAngle = (uint32_t)((PULSE_PER_ONE_TURN_CM36_3650 / 360.0f) * angle + 0.5f);
	
		
	if(Motor.Rotation == Clockwise){
		CurrentPulses = LL_TIM_GetCounter(SPEED_COUNTER_TIM) + PulsesPerAngle;
		if(CurrentPulses > ARR_VALUE_CM36_3650)
		{
			CurrentPulses = CurrentPulses - ARR_VALUE_CM36_3650;
		}
	}
	else{
		CurrentPulses = LL_TIM_GetCounter(SPEED_COUNTER_TIM) - PulsesPerAngle;
		if(CurrentPulses < 0)
		{
			CurrentPulses = ARR_VALUE_CM36_3650 + CurrentPulses;
		}
	}
	
	CM36.CurrentSpeed = speed_percent;
	if(speed_percent <= 30){
...
void CM36_3650_Motor_Voltage_Get(void)
{
